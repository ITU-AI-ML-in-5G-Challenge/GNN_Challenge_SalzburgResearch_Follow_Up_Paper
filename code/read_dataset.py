    ...
    for sample in tqdm.tqdm(it):
        ...
        for i in range(result.shape[0]):
            for j in range(result.shape[1]):
                ...
                avg_bw_complete.append(flow['AvgBw'])
                if flow['ToS'] == 0:
        ...
        if True:
            yield {"bandwith": avg_bw, "bandwith1": avg_bw1, "bandwith2": avg_bw2, 
                "bandwith_complete": avg_bw_complete, "packets": pkts_gen,
...
def input_fn(data_dir, transform=True, repeat=True, shuffle=False):
    ...
    ds = tf.data.Dataset.from_generator(lambda: generator(data_dir=data_dir, shuffle=shuffle),
                                        ({"bandwith": tf.float32,"bandwith1":tf.float32, "bandwith2": tf.float32,  
                                          "bandwith_complete": tf.float32, "packets": tf.float32,
                                        ...
                                        ({"bandwith": tf.TensorShape([None]), "bandwith1": tf.TensorShape([None]),
                                          "bandwith2": tf.TensorShape([None]), "bandwith_complete": tf.TensorShape([None]),
